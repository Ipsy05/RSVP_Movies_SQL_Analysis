...
SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS DatePublished,
SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS Duration,
SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS Country,
SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS WorldWide,
SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) AS Language,
...
FROM movie a INNER JOIN genre b
ON b.movie_id = id
GROUP BY genre
...
SELECT num_genre, COUNT(movie_id) movie_count
FROM (SELECT movie_id, COUNT(genre) AS num_genre
      FROM genre GROUP BY movie_id
      ORDER BY num_genre DESC) num_genre
WHERE num_genre = 1
GROUP BY num_genre;
...
FROM ratings as a INNER JOIN movie AS b
ON b.id = a.movie_id
...
AS (
    SELECT genre, Count(a.id) AS movie_count ,
	   Rank() OVER(ORDER BY Count(a.id) DESC) AS genre_rank
    FROM movie AS a
	   INNER JOIN genre AS b ON b.movie_id = a.id
	   INNER JOIN ratings AS c ON c.movie_id = a.id  
    WHERE avg_rating > 8
...
FROM director_mapping AS d
       INNER JOIN genre b using (movie_id)
...
FROM role_mapping AS rm
       INNER JOIN movie AS mov ON mov.id = rm.movie_id
